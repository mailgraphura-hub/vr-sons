...
const GLOBAL_CSS = `
  ...
  .sc-row:hover { background: ${C.tint10}; box-shadow: inset 0 0 0 1px ${C.tint20}; }
  ...
  .sc-col-header {
    ...
    padding: 10px 24px;
    background: ${C.tint10};
    border-bottom: 1.5px solid ${C.tint20};
  }
  ...
  .sc-add-btn:active { transform: translateY(0) scale(.98); }

  /* ── Toggle ── */
  .sc-toggle {
    position: relative; display: inline-flex;
    height: 24px; width: 44px; align-items: center;
    border-radius: 99px; border: none; cursor: pointer;
    transition: background 250ms ease, box-shadow 200ms ease;
    flex-shrink: 0;
  }
  .sc-toggle:hover { box-shadow: 0 0 0 3px ${C.tint20}; }
  .sc-toggle-thumb {
  ...
  .sc-select {
    appearance: none; background: ${C.tint10};
    border: 1.5px solid ${C.tint20}; color: ${C.text};
  ...
  .sc-select:hover, .sc-select:focus {
    border-color: ${C.tint50}; background: #fff;
    box-shadow: 0 0 0 3px ${C.tint10};
  }

  /* ── Search ── */
  .sc-search-outer { position: relative; flex: 1; min-width: 160px; }
  .sc-search-wrap {
    display: flex; align-items: center; gap: 10px;
    background: ${C.tint10}; border: 1.5px solid ${C.tint20};
  ...
  .sc-suggestions {
    ...
    background: #fff; border: 1.5px solid ${C.tint20};
    border-radius: 14px; box-shadow: 0 12px 36px rgba(195,106,77,.13);
  ...
  .sc-suggestion-item:hover { background: ${C.tint10}; color: ${C.primary}; }

  /* ── Clear btn ── */
  .sc-clear-btn {
    display: flex; align-items: center; gap: 6px;
    padding: 8px 14px; font-size: 12.5px; font-weight: 700;
    color: ${C.muted}; background: ${C.tint10};
    border: 1.5px solid ${C.tint20}; border-radius: 10px; cursor: pointer;
    transition: background 150ms ease, color 150ms ease, transform 180ms ease;
    white-space: nowrap;
  }
  .sc-clear-btn:hover { background: ${C.tint20}; color: ${C.primary}; transform: scale(1.04); }

  /* ── SKU badge ── */
  .sc-sku-badge {
    font-size: 12px; font-family: 'DM Mono', 'Courier New', monospace;
    color: ${C.muted}; background: ${C.tint10};
    border: 1.5px solid ${C.tint20}; padding: 4px 10px;
    border-radius: 8px; white-space: nowrap;
    transition: background 150ms ease, border-color 150ms ease;
  }
  .sc-row:hover .sc-sku-badge { background: ${C.tint20}; border-color: ${C.tint30}; }

  /* ── Pagination btns ── */
  .sc-page-btn {
    display: flex; align-items: center; gap: 6px;
    padding: 7px 14px; font-size: 12.5px; font-weight: 700;
    color: ${C.muted}; background: #fff;
  ...
  .sc-page-btn:not(:disabled):hover {
    background: ${C.tint10}; color: ${C.primary}; border-color: ${C.tint30};
  ...
  .sc-save-btn {
    ...
    color: #fff; background: ${C.primary}; border: none; border-radius: 13px; cursor: pointer;
    box-shadow: 0 4px 14px rgba(195,106,77,.35);
    transition: background 160ms ease, transform 180ms cubic-bezier(.34,1.56,.64,1), box-shadow 180ms ease;
  }
  .sc-save-btn:hover { background: ${C.primary90}; transform: translateY(-2px); box-shadow: 0 8px 20px rgba(195,106,77,.42); }
  .sc-save-btn:active { transform: translateY(0); }

  .sc-cancel-btn {
    padding: 10px 18px; font-size: 13px; font-weight: 700;
    color: ${C.muted}; background: #fff;
    border: 1.5px solid ${C.border}; border-radius: 13px; cursor: pointer;
    transition: background 150ms ease, color 150ms ease, border-color 150ms ease, transform 180ms ease;
  }
  .sc-cancel-btn:hover { background: ${C.tint10}; color: ${C.primary}; border-color: ${C.tint30}; transform: translateY(-1px); }

  .sc-modal-close {
    width: 32px; height: 32px; display: flex; align-items: center; justify-content: center;
    border-radius: 9px; border: none; background: transparent; color: ${C.subtle};
    cursor: pointer; flex-shrink: 0;
    transition: background 150ms ease, color 150ms ease, transform 200ms ease;
  }
  .sc-modal-close:hover { background: ${C.tint10}; color: ${C.primary}; transform: rotate(90deg) scale(1.1); }

  .sc-field {
    width: 100%; padding: 10px 16px; font-size: 13px; color: ${C.text};
    background: #fff; border: 1.5px solid ${C.border}; border-radius: 13px; outline: none;
    transition: border-color 160ms ease, box-shadow 160ms ease;
  }
  .sc-field::placeholder { color: ${C.subtle}; }
  .sc-field:focus { border-color: ${C.tint50}; box-shadow: 0 0 0 3px ${C.tint10}; }

  .sc-upload-label {
    display: flex; align-items: center; gap: 12px; width: 100%;
    padding: 10px 16px; border: 2px dashed ${C.tint30}; border-radius: 13px; cursor: pointer;
    transition: border-color 160ms ease, background 160ms ease, transform 200ms ease;
  }
  .sc-upload-label:hover { border-color: ${C.primary}; background: ${C.tint10}; transform: scale(1.01); }
  ...
  .sc-modal-panel { animation: sc-fade-up 240ms cubic-bezier(.22,1,.36,1) both; }
  ...
  @media (max-width: 640px) {
    ...
    /* Hide 3rd stat card on very small screens */
    .sc-stat-card:nth-child(3) {
      grid-column: 1 / -1;
    }
    ...
    .sc-select-wrap { width: 100%; }
    .sc-select { min-width: unset; }

    /* Hide SKU column in list */
    .sc-col-sku { display: none !important; }

    /* Tighten row grid: remove SKU column */
    .sc-row {
    ...
    /* Outer padding */
    .sc-outer { padding: 0 12px 28px !important; }

    /* Modal → bottom sheet */
    .sc-modal-overlay-inner {
    ...
    .sc-modal-panel {
      ...
      animation: sc-sheet-up 260ms cubic-bezier(.22,1,.36,1) both !important;
    }
    .sc-modal-body   { padding: 16px !important; }
    .sc-modal-header, .sc-modal-footer { padding-left: 16px !important; padding-right: 16px !important; }
    .sc-modal-footer { flex-wrap: wrap; }
    .sc-save-btn, .sc-cancel-btn { flex: 1; text-align: center; justify-content: center; }
  }

  /* ── ≤ 400px: compact ── */
  @media (max-width: 400px) {
    .sc-stats-grid { grid-template-columns: 1fr !important; }
    .sc-stat-card:nth-child(3) { grid-column: auto; }
    .sc-stat-card { padding: 14px 16px; }
  }
`;
...
export default function SubCategories() {
  ...
  return (
    <AdminLayout>
      ...
      <div className="sc-root sc-outer" style={{ background: C.bg, minHeight: "100%", padding: "0 16px 32px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", flexDirection: "column", gap: 22 }}>
          ...
          <div className="sc-filter-bar">
            {/* Parent select */}
            <div className="sc-select-wrap" style={{ minWidth: 160 }}>
              <select
                value={parentFilter}
          ...
          <div style={{
            ...
            <div className="sc-col-header">
              <span style={{ fontSize: 10.5, fontWeight: 800, letterSpacing: "0.08em", textTransform: "uppercase", color: C.subtle }}>Image</span>
              <span style={{ fontSize: 10.5, fontWeight: 800, letterSpacing: "0.08em", textTransform: "uppercase", color: C.subtle }}>Name</span>
              <span className="sc-col-sku" style={{ fontSize: 10.5, fontWeight: 800, letterSpacing: "0.08em", textTransform: "uppercase", color: C.subtle }}>SKU</span>
              <span style={{ fontSize: 10.5, fontWeight: 800, letterSpacing: "0.08em", textTransform: "uppercase", color: C.subtle }}>Status</span>
              <span style={{ fontSize: 10.5, fontWeight: 800, letterSpacing: "0.08em", textTransform: "uppercase", color: C.subtle }}>Action</span>
            </div>

            {subCategories.length === 0 ? (
              <div style={{ padding: "56px 0", textAlign: "center", fontSize: 13, color: C.subtle, fontWeight: 500 }}>
                ...
                {subCategories.map((s, i) => (
                  ...
                  >
                    ...
                    <div className="sc-thumb">
                      {s.subcategoryImage ? (
                        <img src={s.subcategoryImage} alt={s.name} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                      ) : (
                        <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, color: C.subtle }}>N/A</div>
                      )}
                    </div>

                    {/* Name */}
                    <div style={{ minWidth: 0 }}>
                      <p style={{ fontSize: 13.5, fontWeight: 700, color: C.text, margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{s.name}</p>
                      <p style={{ fontSize: 12, color: C.subtle, margin: "3px 0 0", fontWeight: 400, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                        {s.decription?.slice(0, 50)}{s.decription?.length > 50 ? "…" : ""}
                      </p>
                    </div>

                    {/* SKU — hidden on mobile */}
                    <span className="sc-sku-badge sc-col-sku">{s.skuId || "—"}</span>

                    {/* Toggle */}
                    <button
                      className="sc-toggle"
                      onClick={() => toggleStatus(s._id, s.status)}
                      style={{ background: s.status === "Available" ? C.primary : "#d4ccc8" }}
                    >
                      <span
                        className="sc-toggle-thumb"
                        style={{ transform: s.status === "Available" ? "translateX(22px)" : "translateX(3px)" }}
                      />
                    </button>
            ...
            <div style={{
              ...
              padding: "12px 24px", borderTop: `1.5px solid ${C.tint20}`, background: C.tint10,
              flexWrap: "wrap", gap: 8,
            }}>
      ...
      {modal && (
        ...
        >
          <div
            className="sc-modal-overlay-inner"
            style={{
          ...
          >
            <div
              ...
              style={{
                background: C.surface, width: "100%", maxWidth: 460,
                borderRadius: 24,
                boxShadow: "0 24px 64px rgba(195,106,77,.22), 0 4px 12px rgba(0,0,0,.08)",
                overflow: "hidden", border: `1.5px solid ${C.tint20}`,
              }}
            >
              {/* Header */}
              <div
                ...
                style={{
                  display: "flex", alignItems: "center", justifyContent: "space-between",
                  padding: "18px 24px", borderBottom: `1.5px solid ${C.tint20}`,
                  background: C.tint10, gap: 12,
                }}
              >
                <div style={{ minWidth: 0 }}>
                  <h2 style={{ fontSize: 15, fontWeight: 800, color: C.text, margin: 0 }}>Add New Sub‑Category</h2>
                  <p style={{ fontSize: 12, color: C.subtle, marginTop: 3, fontWeight: 500 }}>Fill in the details below</p>
                </div>
                <button className="sc-modal-close" onClick={() => setModal(false)}><X size={16} /></button>
              </div>
              ...
              >
                ...
                <div>
                  ...
                  <div style={{ position: "relative" }}>
                    <select value={parentId} onChange={(e) => setParentId(e.target.value)} className="sc-field sc-select" style={{ width: "100%", minWidth: "unset" }}>
                      <option value="">Select a category</option>
                      {categories.map((cat) => (
                        <option key={cat._id} value={cat._id}>{cat.name}</option>
                      ))}
                    </select>
                    <ChevronRight size={13} style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%) rotate(90deg)", color: C.subtle, pointerEvents: "none" }} />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <input type="text" placeholder="e.g. Smartphones" value={name} onChange={(e) => setName(e.target.value)} className="sc-field" />
                </div>

                {/* SKU */}
                <div>
                  <FieldLabel>SKU ID</FieldLabel>
                  <input type="text" placeholder="e.g. ELEC-PHONE-01" value={skuId} onChange={(e) => setSkuId(e.target.value)} className="sc-field" />
                </div>

                {/* Description */}
                <div>
                  <FieldLabel>Description</FieldLabel>
                  <textarea placeholder="Brief description…" value={description} onChange={(e) => setDescription(e.target.value)} rows={3} className="sc-field" style={{ resize: "none" }} />
                </div>
              ...
              <div
                ...
                style={{
                  ...
                  padding: "14px 24px", borderTop: `1.5px solid ${C.tint20}`, background: C.tint10,
                }}
              >
...
function FieldLabel({ children }) {
  return (
    <label style={{
      ...
      letterSpacing: "0.07em", textTransform: "uppercase", color: C.muted, marginBottom: 6,
    }}>
      {children}
    </label>
  );
...
function StatCard({ icon, label, value, accent, bg, ring }) {
  return (
    <div className="sc-stat-card">
      <div style={{
        height: 46, width: 46, borderRadius: 14, flexShrink: 0,
        display: "flex", alignItems: "center", justifyContent: "center",
        background: bg, border: `1.5px solid ${ring}`, color: accent,
      }}>
        {icon}
      </div>
      <div>
        <p style={{ fontSize: 11, fontWeight: 800, letterSpacing: "0.08em", textTransform: "uppercase", color: C.subtle, margin: 0 }}>{label}</p>
        <p style={{ fontSize: 26, fontWeight: 800, color: C.text, margin: "2px 0 0", lineHeight: 1.1 }}>{value ?? "—"}</p>
      </div>
    </div>
  );
}